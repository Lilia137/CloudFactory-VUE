<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item style="font-weight: bold">生产中订单</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图 -->
    <el-card>
      ...
      <!-- 表格数据 -->
      <el-table :data="produceOrderList" stripe>
        ...
        <el-table-column align="right">
          ...
          <template slot="header" slot-scope="scope">
            <el-input placeholder="输入关键词进行检索" v-model="queryInfo.query" size="mini" clearable @clear="getProduceOrderList">
              <el-button slot="append" icon="el-icon-search" @click="getProduceOrderList()"></el-button>
            </el-input>
          </template>
          <template slot-scope="props">
...
export default {
  ...
  created () {
    const that = this
    that.getProduceOrderList()
    console.log('coming data:', that.produceOrderList)
  },
  methods: {
    // 根据分页获取对应的商品列表
    async getProduceOrderList () {
      const that = this
      var u = '/getProduceOrder' + '?' + 'query='
      ...
      that.$http({
        ...
        console.log('res', res)
        that.produceOrderList = res.data[0].list
        // 分页
        // that.total = res.data.total
        console.log(that.produceOrderList)
      }).catch(function (res) {
        console.log(res)
